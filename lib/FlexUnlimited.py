        ...
        try:
            with open("config.json") as configFile:
                ...
                self.desiredWarehouses = config["desiredWarehouses"] if len(
                    config["desiredWarehouses"]) >= 1 else None  # list of warehouse ids
                self.minBlockRate = config["minBlockRate"]
                self.arrivalBuffer = config["arrivalBuffer"]
                # start hour in military time
                self.desiredStartHour = config["desiredStartHour"]
                # end hour in military time
                self.desiredEndHour = config["desiredEndHour"]
